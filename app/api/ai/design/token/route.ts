...
/** POST — mint a Trigger.dev public token scoped to one run (after TaskRun ownership check). */
export async function POST(request: Request) {
  ...
  try {
    ...
    console.error("[design/token] Prisma direct client (TaskRun) unavailable", configErr);
    return Response.json(
      {
        error:
  ...
  try {
    ...
    console.error("[design/token] auth.createPublicToken failed", err);
    return Response.json(
